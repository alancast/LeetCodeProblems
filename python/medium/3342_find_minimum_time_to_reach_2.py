...
class Solution:
    ...
    def minTimeToReach(self, moveTime: List[List[int]]) -> int:
        ...
        while min_heap:
            ...
            # See if this move requires 1 or 2 seconds to get there
            # Could also do by adding a variable to the state class
            move_speed = ((state.x + state.y) % 2) + 1

            # Check and add new entries to queue
            # Above
            if state.y - 1 >= 0 and moveTime[state.y - 1][state.x] != -1:
                next_distance = max(state.distance + move_speed, moveTime[state.y - 1][state.x] + move_speed)
            ...
            if state.x - 1 >= 0 and moveTime[state.y][state.x - 1] != -1:
                next_distance = max(state.distance + move_speed, moveTime[state.y][state.x - 1] + move_speed)
